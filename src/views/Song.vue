  ...
  <main>
    ...
    <section class="w-full mb-8 py-14 text-center text-white relative">
      ...
      <div class="container mx-auto flex items-center">
        <!-- Play/Pause Button -->
        <button type="button" @click.prevent="newSong(song)" class="z-50 h-24 w-24 text-3xl bg-white text-black rounded-full focus:outline-none">
    ...
    <section class="container mx-auto mt-6" id="comments">
      <div class="bg-white rounded border border-gray-200 relative flex flex-col">
        ...
        <div class="p-6">
          <div :class="comment_alert_variant" v-if="comment_show_alert" class="text-white text-center font-bold p-4 mb-4">
          ...
          <vee-form v-if="userLoggedIn" @submit="addComment" :validation-schema="schema">
            ...
            <button :disabled="comment_in_submission" type="submit" class="py-1.5 px-3 rounded text-white bg-green-600 block">Submit</button>
          </vee-form>
          <!-- Sort Comments -->
...
export default {
  ...
  computed: {
    ...
    ...mapGetters(["playing"]),
    sortedComments() {
      return this.comments.slice().sort((commentA, commentB) => {
        if (this.sort === "1") {
          return new Date(commentB.datePosted) - new Date(commentA.datePosted);
        }
        return new Date(commentA.datePosted) - new Date(commentB.datePosted);
      });
    },
  },
  watch: {
